...
- **GKE fails** — the hard direction, because the active hub is the casualty. **Current limitation:** the composition adds the Tailscale exposure annotations only on GKE (`{{- if eq $env "gke" }}`), so scaling up the homelab standby *alone* would not make it tailnet-reachable — homelab failover also requires extending the composition/claim to expose homelab over the tailnet (deferred along with homelab exposure — see Device naming). Once that lands, the v1 manual path is: scale up the homelab standby (`replicas: 0` → `1`), the phone receives via the homelab topic over the tailnet, and duplicate notifications during the incident window are acceptable. Phase B (Uptime Kuma arc): a homelab watchdog probes GKE ntfy `/v1/health` and auto-activates the standby on failure.
