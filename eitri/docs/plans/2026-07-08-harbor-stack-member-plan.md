...
- [ ] **Step 2 [HUMAN]: Tear down the direct install.** `helm --kube-context <hub> uninstall harbor -n harbor`. Leave the `harbor` namespace. **This is a destructive re-provision, not a data migration:** a fresh Mimir-vended Postgres starts empty, so Harbor's DB metadata (projects, proxy-cache config, users, robots) is NOT carried over — only the cached blobs re-warm on demand. Safe for THIS instance because it's proxy-cache-only + public-read: the composition's setup Job re-creates the five proxy-cache projects, and there are no pushed images, users, or robot accounts to preserve. (If this Harbor ever holds pushed images or real accounts, add a Harbor DB export/restore around the cutover instead.)
